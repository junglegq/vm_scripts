#!/bin/bash
#
# /oses/vm_scripts/createVMs.sh
# 
# NOTE: current implementation will identify the hostname for the config policy. Be aware. 
#
# What to do:
# 1. Generate /oses/mapping.csv
# 2. Generate paipai_<hostname>.conf
# 3. Upload paipai_<hostname>.conf to DHCP server and make it work.
# 4. Create VMs 
...
DHCPSERVER=10.151.0.2
IPPREFIX="10.151.0."

_prefixVM="00:16:3e"
_prefixSubIP2="97"   ## 0x97 = 0d151
_prefixSubIP3="00"
PrefixMAC=${_prefixVM}":"${_prefixSubIP2}":"${_prefixSubIP3}
...
# Test Only, to Pin CPU to target VM
	...
	if [[ $thisbox = "box0" || $thisbox = "box1" || $thisbox = "node3" ]]; then
		echo "vif = [ 'type=ioemu, mac=$mac, bridge=br151' ]" >> $CURCONF
...
#	int g0/0: 10.151.0.0/24 (box0, box1, node3)
...
# 	nat server protocol tcp global current-interface 59103 inside 10.151.0.103 5900
# Or, 
